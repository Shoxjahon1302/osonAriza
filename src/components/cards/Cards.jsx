...
export default function Cards() {
  ...
  return (
    <div className="flex flex-wrap gap-4">
      {data.map((item) => (
        <div className="rounded-md bg-[#f8f8f8] odd:bg-green-50 py-3 w-[200px] min-h-[150px] text-center p-4 flex items-center flex-col justify-center">
          <h1 className="text-4xl font-['Inter'] font-medium">{item.number}</h1>
          <h1 className="text-md font-['Inter'] font-medium">{item.title}</h1>
        </div>
      ))}
    </div>
  );
